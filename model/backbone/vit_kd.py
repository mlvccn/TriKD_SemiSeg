...
class DropPath(nn.Module):
    ...
    def forward(self, x):
        ...
        # 由于随机保留了一部分张量内容，导致整体期望降低，所以在原有的取值范围内造成的影响比以前更大，为了消除这样的弊端所以除以keep_prob保持期望不变
...
if __name__ == '__main__':
    ...
    # )
    # model = VisionTransformer(
    #     model_name='vit_small_patch16_224', # config: https://github.com/rwightman/pytorch-image-models/blob/5f9aff395c224492e9e44248b15f44b5cc095d9c/timm/models/vision_transformer.py#L490
    #     img_size=768,
    #     patch_size=16,
    #     in_chans=3,
    #     embed_dim=768,
    ...
    model = VisionTransformer(
        model_name='vit_deit_small_distilled_patch16_224', # config: https://github.com/rwightman/pytorch-image-models/blob/v0.4.5/timm/models/vision_transformer.py#L758
        img_size=224,
        patch_size=16,
        in_chans=3,
        embed_dim=384,
    ...
    gpu=2
    # model.cuda(gpu)
    img = torch.randn(1,3,224,224)#.cuda(gpu)
    preds, attns = model(img)
    print(len(preds))
    for pred, attn in zip(preds, attns):
