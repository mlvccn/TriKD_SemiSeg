...
def load_state_multi_optimizer(path, model, optimizer=None, key="state_dict", optimizer_str="optimizer"):
    rank = dist.get_rank()
    ...
    if os.path.isfile(path):
        ...
        if optimizer is not None:
            best_metric = checkpoint["best_miou"]
            last_iter = checkpoint["epoch"]
            optimizer.load_state_dict(checkpoint[optimizer_str])
            if rank == 0:
                print(
                    "=> also loaded optimizer from checkpoint '{}' (epoch {})".format(
                        path, last_iter
...
def create_pascal_label_colormap():
    ...
    # colormap[0] = [0, 0, 0] # all black 
    colormap[0] = [200,200,200] # gray
    colormap[1] = [128, 0, 0]
    colormap[2] = [0, 128, 0]
